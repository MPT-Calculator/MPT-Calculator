...
def measure_memory_usage(n_cpus=1, order=0):
    ...
    options_dict = {'order': order, 'geometry': geo, 'use_OCC': True, 'use_POD': True, 'cpus': n_cpus, 'alpha': 0.01,
...
def plot_times(Datadir: str, n_cpus: int):
    ...
    timing_list = [timings['start_time'] - function_call_time,
                    ...
                    timings['Theta1'] - function_call_time,
                    timings['ROM'] - function_call_time,
                    timings['SolvedSmallerSystem'] - function_call_time,0,
                    timings['Tensors'] - function_call_time]
    ...
    #plt.legend(['Off-line', b1, b2, b3, b4, 'On-line',b5, b6, b7], ['', 'Computing Mesh', r'Computing $\mathbf{o}_i(\mu_r)$', r'Computing $\mathbf{q}_i(\omega_k)$ Snapshots', r'TSVD \& Constucting Smaller System', '', 'Solving Smaller System', 'Building Matrices', 'Computing MPT Spectral Signature' ],
    #    handler_map={str: LegendTitle({'fontsize': 11})})
    
    plt.legend(['Off-line', b1, b2, b3, b4, 'On-line',b5, b7], ['', 'Computing Mesh', r'Computing $\mathbf{o}_i(\mu_r)$', r'Computing $\mathbf{q}_i(\omega_k)$ Snapshots', r'TSVD \& Constucting Smaller System', '', 'Solving Smaller System', 'Computing MPT Spectral Signature' ],
        handler_map={str: LegendTitle({'fontsize': 11})})
    plt.ylabel('Memory Usage, [MB]')
    plt.xlabel('Time, [sec]')
...
if __name__ == '__main__':
    ...
    plt.savefig('Results/' + ret['SweepName'] + '/Graphs/'+'cpu_timing.pdf')
    plt.show()

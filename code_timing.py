...
def measure_memory_usage(n_cpus=2, order=3):
    """
    James Elgy 2024
    ...
    Memory usage and timings are outputted to timings_cpus_{n_cpus} and memory_usage_cpus_{n_cpus} files.
    ...
    """
    geo = 'OCC_test_sphere_prism_32.py'
    options_dict = {'order': order, 'geometry': geo, 'use_OCC': True, 'use_POD': True, 'cpus': n_cpus, 'alpha': 0.001,
                            'use_parallel': True, 'use_iterative_POD': False, 'start_stop':(1,8,81)}
    ...
    np.save('Results/' + ret['SweepName'] + f'/Data/timings_cpus_{n_cpus}.npy', timings)
    np.save('Results/' + ret['SweepName'] + f'/Data/ReturnDict.npy', ret)
    np.save('Results/' + ret['SweepName'] + f'/Data/memory_usage_{n_cpus}.npy', [mem, time_array])
...
def plot_times(Datadir: str, n_cpus: int):
    ...
    times = np.load(Datadir + f'timings_cpus_{n_cpus}.npy', allow_pickle=True).all()
    mem = np.load(Datadir + f'memory_usage_{n_cpus}.npy', allow_pickle=True)
...
if __name__ == '__main__':
    mem, timings, function_call_time, time_array, ret = measure_memory_usage(n_cpus=4, order=0)
    Datadir = 'Results/' + ret['SweepName'] + '/Data/'
    plot_times(Datadir, 4)
    plt.show()

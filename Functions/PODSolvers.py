...
def PODSweep_mur(Object, Order, alpha, inorout, mur, sig, Array, PODArray, PODTol, PlotPod, sweepname, SavePOD,
             PODErrorBars, BigProblem):

    PODArray_mur = np.logspace(0,2,6)
    Array_mur = np.logspace(0,2,30)
    ...
    # Scalars
    Mu0 = 4 * np.pi * 10 ** (-7)
    NumberofSnapshots = len(PODArray) * len(PODArray_mur)
    NumberofFrequencies = len(Array) * len(Array_mur)
    # Coefficient functions
    mu_coef = [mur[mat] for mat in mesh.GetMaterials()]
    mu = CoefficientFunction(mu_coef)
    inout_coef = [inorout[mat] for mat in mesh.GetMaterials()]
    ...
    R_Snapshots = np.zeros((3,3,len(PODArray), len(PODArray_mur)))
    I_Snapshots = np.zeros((3,3,len(PODArray), len(PODArray_mur)))
    N0_Snapshots = np.zeros((3,3, len(PODArray_mur)))


    for ind, mur in enumerate(PODArray_mur):
        #########################################################################
        # Theta0
        # This section solves the Theta0 problem to calculate both the inputs for
        # the Theta1 problem and calculate the N0 tensor

        mu_coef = [{'air':1, 'sphere':mur}[mat] for mat in mesh.GetMaterials()]
        mu = CoefficientFunction(mu_coef)
        ...
        # if ind == 0:
        #     N0_array = N0[:,:,None]
        #     Theta0Sol_array = Theta0Sol[:,:,None]
        # else:
        #     N0_array = np.concatenate((N0_array, N0[:,:,None]), axis=-1)
        #     Theta0Sol_array = np.concatenate((Theta0Sol_array, Theta0Sol[:,:,None]),axis=-1)
        ...
        if BigProblem == True:
            Theta1Sols = np.zeros([ndof2, len(PODArray), 3], dtype=np.complex64)
        else:
            Theta1Sols = np.zeros([ndof2, len(PODArray), 3], dtype=complex)
        ...
        if ind == 0:
            Theta1Sols_array = Theta1Sols
        else:
            Theta1Sols_array = np.concatenate((Theta1Sols_array, Theta1Sols), axis=1)

        #########################################################################
        # Calculating MPT for snapshot parameters
        for k, omega in enumerate(PODArray):
            ...
            for i in range(3):
                ...
                xii = xivec[i]
                if i == 0:
                    Theta1i.vec.FV().NumPy()[:] = Theta1Sols[:, k, i]
                if i == 1:
                    Theta1i.vec.FV().NumPy()[:] = Theta1Sols[:, k, i]
                if i == 2:
                    Theta1i.vec.FV().NumPy()[:] = Theta1Sols[:, k, i]
                for j in range(i + 1):
                    Theta0j.vec.FV().NumPy()[:] = Theta0Sol[:, i]
                    xij = xivec[j]
                    if j == 0:
                        Theta1j.vec.FV().NumPy()[:] = Theta1Sols[:, k, i]
                    if j == 1:
                        Theta1j.vec.FV().NumPy()[:] = Theta1Sols[:, k, i]
                    if j == 2:
                        Theta1j.vec.FV().NumPy()[:] = Theta1Sols[:, k, i]
                    ...
                    R[i, j] = -(((alpha ** 3) / 4) * Integrate(
                        (mu ** (-1)) * (curl(Theta1j) * Conj(curl(Theta1i))), mesh)).real
                    I[i, j] = ((alpha ** 3) / 4) * Integrate(
                        inout * nu * sigma * ((Theta1j + Theta0j + xij) * (Conj(Theta1i) + Theta0i + xii)),
                        mesh).real
            R += np.transpose(R - np.diag(np.diag(R))).real
            I += np.transpose(I - np.diag(np.diag(I))).real
            R_Snapshots[:, :, k, ind] = R
    ...
    # To avoid changing too much code, we overwrite the theta1 vector with the extended version.
    Theta1Sols = Theta1Sols_array
    ...
    if BigProblem == True:
        ain = BilinearForm(fes2, symmetric=True)
        aout = BilinearForm(fes2, symmetric=True)
    else:
        ain = BilinearForm(fes2)
        aout = BilinearForm(fes2)

    # Here a0 is split into inside and outside the object as ain and aout
    ain += SymbolicBFI(inout * InnerProduct(curl(u), curl(v)))
    aout += SymbolicBFI((1-inout) * InnerProduct(curl(u), curl(v)))
    aout += SymbolicBFI((1j) * (1 - inout) * epsi * InnerProduct(u, v))
    ...
    # E1
    # Similarly to splitting a0, we construct this new matrix A0outH
    for i in range(cutoff):
        read_vec.FV().NumPy()[:] = u1Truncated[:, i]
    ...
    if PODErrorBars == True:
        ...
        RerrorReduced1[:, 0] = ProL.vec.FV().NumPy()[:]
        for i in range(cutoff):
            ProH.vec.FV().NumPy()[:] = A0H_1[:, i]
            ProL.Set(ProH)
            RerrorReduced1[:, i + 1] = ProL.vec.FV().NumPy()[:]
            ProH.vec.FV().NumPy()[:] = A1H_1[:, i]
            ProL.Set(ProH)
            RerrorReduced1[:, i + cutoff + 1] = ProL.vec.FV().NumPy()[:]
    # E2
    for i in range(cutoff):
    ...
    if PODErrorBars == True:
        ...
        RerrorReduced2[:, 0] = ProL.vec.FV().NumPy()[:]
        for i in range(cutoff):
            ProH.vec.FV().NumPy()[:] = A0H_2[:, i]
            ProL.Set(ProH)
            RerrorReduced2[:, i + 1] = ProL.vec.FV().NumPy()[:]
            ProH.vec.FV().NumPy()[:] = A1H_2[:, i]
            ProL.Set(ProH)
            RerrorReduced2[:, i + cutoff + 1] = ProL.vec.FV().NumPy()[:]
    # E3
    for i in range(cutoff):
    ...
    if PODErrorBars == True:
        ...
        RerrorReduced3[:, 0] = ProL.vec.FV().NumPy()[:]
        for i in range(cutoff):
            ProH.vec.FV().NumPy()[:] = A0H_3[:, i]
            ProL.Set(ProH)
            RerrorReduced3[:, i + 1] = ProL.vec.FV().NumPy()[:]
            ProH.vec.FV().NumPy()[:] = A1H_3[:, i]
            ProL.Set(ProH)
            RerrorReduced3[:, i + cutoff + 1] = ProL.vec.FV().NumPy()[:]
    ...
    N0_array = np.zeros((3,3,len(Array_mur)))
    R_array = np.zeros((3,3,len(Array), len(Array_mur)))
    I_array = np.zeros((3,3,len(Array), len(Array_mur)))
    count = 0

    ### EVALUATE THE ROM ###

    for ind, mur in enumerate(Array_mur):
        # In the absence of a theta0 ROM, we simply calculate theta0 directly for each value of mur.
        mur_dict = {'air': 1.0, 'sphere': mur}
        mu = CoefficientFunction([mur_dict[mat] for mat in mesh.GetMaterials()])
        ...
        for k, omega in enumerate(Array):
            ...
            # This part is for obtaining the solutions in the lower dimensional space
            print(' solving reduced order system %d/%d    ' % (count, NumberofFrequencies), end='\r')
            t1 = time.time()
            ...
            for i in range(3):
                ...
                for j in range(i + 1):
                    ...
                    R[i, j] = -(((alpha ** 3) / 4) * Integrate((mu ** (-1)) * (curl(Theta_1j) * Conj(curl(Theta_1i))),
                                                               mesh)).real
                    I[i, j] = ((alpha ** 3) / 4) * Integrate(
                        inout * nu * sigma * ((Theta_1j + Theta_0j + xij) * (Conj(Theta_1i) + Theta_0 + xii)), mesh).real
    ...
    xx, yy = np.meshgrid(np.log10(Array), np.log10(Array_mur))
    plt.figure()
    ax = plt.axes(projection='3d')
    ax.plot_wireframe(xx, yy, I_array[0,0,:,:].transpose())
    plt.title('Imag')

    N0_2 = np.zeros(R_array.shape)
    for ind in range(len(Array)):
        N0_2[:,:,ind,:] = N0_array
    plt.figure()
    ax = plt.axes(projection='3d')
    ax.plot_wireframe(xx, yy, R_array[0,0,:,:].transpose() + N0_2[0,0,:,:].transpose())
    plt.title('Real + N0')

...
def Thetainf(fes, Order, alpha, inout, e, xivec, Tolerance, Maxsteps, epsi, simnumber, Solver, Additional_Int_Order, mesh, Dirnotouterstring, use_longdouble=True):
    """
    P.D. Ledger.
    Function to compute theta inf solution vectors.
    Note: previously this function also computed N0, and there are some leftover arguments.

    Args:
        fes (comp.HCurl): HCurl finite element space for the Thetainf problem.
        ...
        Dirnotouterstring: string of boundaries apart from outer for applying Dirichlet condition - note may not just be default
    ...
    #Theta.Set((0, 0, 0), BND)
    Theta.Set(xivec, definedon=mesh.Boundaries(Dirnotouterstring))
    ...
    f = LinearForm(fes)
    #f += SymbolicLFI(-(1-inout)*InnerProduct(e, curl(v)), bonus_intorder=Additional_Int_Order)
    a = BilinearForm(fes, symmetric=True, condense=True)
    a += SymbolicBFI( curl(u) * curl(v), bonus_intorder=Additional_Int_Order)

...
def PODSweep(Object, Order, alpha, inorout, mur, sig, Array, PODArray, PODTol, PlotPod, sweepname, SavePOD,
             PODErrorBars, BigProblem, NumSolverThreads, Integration_Order, Additional_Int_Order, Order_L2, curve=5, recoverymode=False, save_U=False):
    ...
    if use_integral is True or use_integral_debug is True:
        ...
        for i in range(Tensor_CPUs):
            runlist.append((Core_Distribution[i], Q_array, c1_array, c5_array, c7, c8_array, At0_array, UAt0_conj,
                            UAt0U_array, T_array, EU_array_conj, UH_array, Lower_Sols[i], [], cutoff,
                            fes2.ndof,
                            alpha, False))

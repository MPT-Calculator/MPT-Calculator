...
def lsq_linear_refined(A, b, bounds=(0, np.inf), max_iter=0): #10

    print("without preconditioning",np.linalg.cond(A))
    # 0. Precondition columns to have unit norm
    col_norms = np.linalg.norm(A, axis=0)
    col_norms[col_norms == 0] = 1.0  # Prevent division by zero
    A = A / col_norms
    print("with preconditioning",np.linalg.cond(A))
    ...
    for n in range(max_iter):
        ...
        # 4. Solve for the correction dx using the residual as the target# method ='bvls'

...
#Edit strings
def FloattoStringPlot(value,tick_number):
    value = value.item()
    print(value)
    print(type(value))
    for i in range(100):
        if value>=1:
            ...
            print("rhuh rho")
            print(abs(value/(10**(-i))))
            #if value/(10**-(i-1))>=1:
            if abs(value/(10**(-i)))>=1:
                power=-(i-1)
                break
    StringNumber=str(value/(10**power))
    SnipNumber=StringNumber[:4]+"e"+str(power)
    return SnipNumber

def FTSP(value,tick_number):
    if value==0:
        newvalue = "0"
    elif value==1:
        newvalue = "1"
        ...
        #    #if StringNumber[4]=="0":
        #    #    if StringNumber[3]=="0":
        #    #        newvalue=StringNumber[:2]+"e"+str(power)
        #    #    else:
        #    #        newvalue=StringNumber[:4]+"e"+str(power)
        #    #else:
        #    newvalue=StringNumber[:5]+"e"+str(power)
        #else:
        #    newvalue=StringNumber[:4]+"e"+str(power)
    return newvalue
...
def MultiPlotterSaver(Plotname,xs,ys,numberoflines,styles,colours,names,xlabel,ylabel,xlog,ylog,title,show):
    ...
    for l,line in enumerate(xs):
        ...
        for i in range(linestoplot):
            if linestoplot==1:
                if l==0 and i==0:
                    lines = ax.plot(x,y,style[i],markersize=6,color=PYCOL[int(colour[i])-1])
                else:
                    lines += ax.plot(x,y,style[i],markersize=4,color=PYCOL[int(colour[i])-1])
            else:
                if l==0 and i==0:
                    lines = ax.plot(x,y[:,i],style[i],markersize=8,color=PYCOL[int(colour[i])-1])
                else:
                    lines += ax.plot(x,y[:,i],style[i],markersize=6,color=PYCOL[int(colour[i])-1])
    ...
    ax.yaxis.set_major_formatter(plt.FuncFormatter(FTSP))
    plt.subplots_adjust(wspace=0.6, hspace=0.6, left=0.16, bottom=0.1, right=0.94, top=0.90)
